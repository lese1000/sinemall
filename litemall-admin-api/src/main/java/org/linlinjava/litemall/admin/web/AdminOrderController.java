...
public class AdminOrderController {
    ...
    public Object refund(@LoginAdmin Integer adminId, @RequestBody String body) {
        ...
        try {
            ...
            //商户系统内部订单号（transactionId微信订单号，二选一）
//            request.setTransactionId(transactionId);
            request.setOutTradeNo(order.getOrderSn());
            //商户系统内部的退款单号，商户系统内部唯一(未做存储)
            request.setOutRefundNo(getOutRefundNo());
            //订单金额
            request.setTotalFee(order.getOrderPrice().multiply(new BigDecimal(100)).intValue());
            //退款金额
            request.setRefundFee(order.getActualPrice().multiply(new BigDecimal(100)).intValue());
			WxPayRefundResult refundResult = wxPayService.refund(request);
			if(!refundResult.getResultCode().equals("SUCCESS")) {
				txManager.rollback(status);
				logger.error("微信申请退款失败常:" + refundResult.getXmlString());
				return ResponseUtil.fail(403, "微信申请退款失败");
			}
		} catch (Exception e) {
			txManager.rollback(status);
			logger.error("微信商户申请退款异常", e);
			return ResponseUtil.fail(403, "微信商户申请退款异常");
		}
    ...
    public Object wechatRefundQuery(@LoginAdmin Integer adminId, @RequestBody String body) {
    	if (adminId == null) {
            return ResponseUtil.unlogin();
        }
        Integer orderId = JacksonUtil.parseInteger(body, "orderId");
        ...
        try {
			WxPayRefundQueryResult refundQueryResult = wxPayService.refundQuery("", order.getOrderSn(), "", "");
			return ResponseUtil.ok(refundQueryResult);
		} catch (WxPayException e) {
			logger.error("查询微信退款异常", e);
			return ResponseUtil.fail(409, "查询微信退款异常");
		}
    }
    ...
    public static String getOutRefundNo() {
        ...
        tradeno = "RF" + tradeno + str.substring(str.length() - 6);
        return tradeno;
    }
}
